#!/bin/bash
# D1-v2: single-session, single-state definitive 0 V spectrum, x3 passes.
...
log(){ echo "[$(date +%H:%M:%S)] $*"; }
for p in 1 2 3; do
  log "D1v2 pass $p/3: manual lows"
  .venv/bin/python /tmp/manual_lowf_v2.py "V2P$p" || exit 1
  log "D1v2 pass $p/3: sweeper 34-340"
  $RUN $F --no-contact-check || exit 1
  log "D1v2 pass $p/3: wideband"
